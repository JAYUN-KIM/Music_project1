...
# ★ Ansible 환경 변수 주입 (기본값 설정 포함)
# Ansible에서 실 배포 시 주입한 값들이 최우선 적용됩니다.
...
def analyze():
    if not os.path.exists(INPUT_AUDIO):
        print(f"[ERROR] 입력 파일({INPUT_AUDIO})이 없습니다.")
        return

    try:
        # --- 0. LB 서버 정적 파일 초기화 (Frontend Cache 방지) ---
        print(f">>> [0/5] LB 서버({LB_HOST_IP}) 파일 정리 중...")
        ...
        # --- 3. 감정 매핑 및 리소스 생성 ---
        print(">>> [3/5] 감정 랭킹 추출 및 테마 데이터 생성...")
        ...
        palette = {
            "main": f"hsl({hue:.1f}, {sat}%, 45%)",
            "bg": f"hsl({hue:.1f}, 20%, 8%)",
            "accent": f"hsl({(hue+150)%360:.1f}, 90%, 50%)"
        }
        ...
        nas_file_name = f"music_analysis_{ts}.tar.gz"
